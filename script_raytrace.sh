#!/bin/bash
# raytrace: full pipeline for the HW/SW co-design project.
#
#   1. environment setup and dependency installation
#   2. correctness check (optimized versions render the same image)
...
#
# usage:  sudo ./script_raytrace.sh
#
# Environment variables:
#   SKIP_SETUP=1     skip step 1 (dependencies already installed)
#   ALL_VARIANTS=1   also benchmark every single optimization (shadow_ray,
#                    sphere_floats, slots, combined), not only the final one
...
#
# The final optimized version is raytrace/optimized/combined:
# - the shadow ray in _lightIsVisible is built once, not once per object
# - Sphere.intersectionTime uses plain floats instead of temporary Vectors
# - Vector and Point use __slots__ (no per-object dictionary)
set -euo pipefail

BENCH=raytrace
FINAL=combined
VARIANTS=(shadow_ray sphere_floats slots combined)
...
run python3 "$REPO/raytrace/verify_raytrace.py" | tee "$OUT/verify.txt"
