...
class Vector(object):
    # No per-object __dict__: cheaper to create, faster attribute access.
    __slots__ = ('x', 'y', 'z')
...
class Sphere(object):
    ...
    def intersectionTime(self, ray):
        # Same math and order as the Vector version, but with plain floats:
        # no temporary Vector, no dot()/__sub__/isPoint/mustBeVector calls.
        centre = self.centre
        point = ray.point
        vector = ray.vector
...
class Scene(object):
    ...
    def _lightIsVisible(self, l, p):
        # The shadow ray is the same for every object: build it once.
        ray = Ray(p, l - p)
        for (o, s) in self.objects:
